...
export const transformerFileName = ({
  ...
  pre(node) {
    ...
    node.children.push({
      ...
      properties: {
        class: [
          "absolute py-1 text-ink text-xs font-medium leading-4",
          hideDot
            ? "px-2"
          ...
          style === "v1"
            ? "left-0 -top-6 rounded-t-md border border-b-0 bg-surface/50"
            : "left-2 top-(--file-name-offset) border rounded-md bg-paper",
        ],
      },
      children: [
        {
