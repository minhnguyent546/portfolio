---
title: XOR Basis
description: Giới thiệu kỹ thuật xor basis — biểu diễn số nguyên thành vector trong không gian vector nhị phân và dùng cơ sở của không gian đó để giải các bài toán liên quan đến tổng xor của các phần tử trong mảng.
tags:
  - xor-basis
  - linear-algebra
topic: algorithms
...
Một tập các vector $\{v_1, v_2, \ldots, v_n\}$ được gọi là **độc lập tuyến tính** nếu phương trình:

$$
x_1v_1 + x_2v_2 + \ldots + x_nv_n = O
$$

chỉ có duy nhất một nghiệm tầm thường $x_1 = x_2 = \ldots = x_n = 0$.

Ngược lại nếu tồn tại $x_1, x_2, \ldots, x_n$ không đồng thời bằng không, sao cho:
...
_Chú ý:_ do chỉ xét trên $\mathbb{Z}_2^d$ nên $c_i$ cũng phải thuộc $\mathbb{Z}_2^d$. Do đó trong tổ hợp trên mỗi $v_i$ chỉ có hai trạng thái là **xuất hiện** hoặc **không xuất hiện**.
**Ví dụ:** tìm span$(\{2, 5\})$, span$(\{2, 5, 7\})$ và span$(\{\})$?
...
Một tập các vector $B = \{v_1, v_2, \ldots, v_n\}$ được gọi là cơ sở của một không gian vector $V$ nếu span$(B) = V$ và $B$ là **độc lập tuyến tính**. Khi đó $n$ được gọi là **số chiều** của $V$ và kí hiệu là $\text{dim}(V)$.
...
Không gian hạch của một ma trận $M$, kí hiệu là null$(M)$ là tập các vector $x \in \mathbb{R}^m$ (được viết ở dạng cột) sao cho:
...
**Đặc biệt:** nếu ma trận $M$ kích cỡ $n \times m$ gồm $m$ vector $v \in \mathbb{Z}_2^n$ và vector $x \in \mathbb{Z}_2^m$ (các vector $v$ và $x$ được viết ở dạng cột):
...
Để ý rằng chúng ta cần thỏa mãn điều kiện $\forall x, y \in a$ thì $x \oplus y \in a$. Do đó cần phải xây dựng được **cơ sở $B$** của không gian vector $V$ được span bởi mảng $a$. Khi đó đáp án của bài toán chính là $2^{\text{dim}(\text{span}(a))} - n = 2^{|B|} - n$.
...
- Nếu xem xét tất cả $2^{|B|}$ tổ hợp tuyến tính của các vector trong $B$ thì rõ ràng sẽ không đủ thời gian.
...
- **Tổng quát:** thuật toán trên cho phép tìm giá trị nhỏ nhất của $(a_i + v)$ với $v \in \text{span}(\{b_1, b_2, \ldots, b_k\})$.
- **Ví dụ:** xét $x = 9$ và $\text{span}(\{26, 15, 3, 1\})$. Khi đó giá trị nhỏ nhất có thể đạt được là:

  $$
  9 \oplus 15 \oplus 3 \oplus 1 = \mathbf{4} = 100_2.
  $$
...
- Chúng ta cần tìm tất cả các vector $x = [x_1, x_2, \ldots, x_n]$ ($x \in \mathbb{Z}_2^n$), trong đó $x_i = 0$ tương ứng với $a_i$ không được chọn và $x_i = 1$ tương ứng với $a_i$ được chọn, sao cho:
...
- Theo định lí về hạng và số vô hiệu:
  ...
  $$
  \text{nullity}(M) = n - \text{rank}(M) = n - \text{dim}(\text{CS}(M)) = n - \text{dim}(\text{span}(a))
  $$
...
<!-- - Tìm các số nguyên tố: $\mathcal{O}(m\sqrt{m}$.
- Tìm các vector tương ứng: $\mathcal{O}(md\log{m})$.
- Tính kết quả: $\mathcal{O}(n)$. -->
...
- Khi đó ta cần tìm số vector $x = [x_1, x_2, \ldots, x_n]$ ($x \in \mathbb{Z}_2^n$) sao cho:
  ...
  \Longleftrightarrow
  \begin{cases}
  	a_{1, 1}x_1 + a_{2, 1}x_2 + \ldots + a_{n, 1}x_n \equiv a_{i, 1} + a_{i + 1, 1} + \ldots + a_{j, 1} \pmod 2 \\
  	a_{1, 2}x_1 + a_{2, 2}x_2 + \ldots + a_{n, 2}x_n \equiv a_{i, 2} + a_{i + 1, 2} + \ldots + a_{j, 2} \pmod 2 \\
  	\ldots \\
  	a_{1, d}x_1 + a_{2, d}x_2 + \ldots + a_{n, d}x_n \equiv a_{i, d} + a_{i + 1, d} + \ldots + a_{j, d} \pmod 2
  \end{cases}
  $$
