---
title: Auxiliary Tree/Virtual Tree (cây phụ/cây ảo)
description: Kĩ thuật giải quyết các bài toán truy vấn trên một tập đỉnh của cây.
tags:
  - auxiliary-tree
  - virtual-tree
topic: algorithms
...
![Cây ban đầu với các đỉnh truy vấn được tô màu đỏ](./auxiliary-tree/original-tree.svg)

<figcaption><em>Cây ban đầu</em></figcaption>
</figure>
<figure>

![Cây phụ gồm các đỉnh truy vấn màu đỏ và các đỉnh LCA màu xanh](./auxiliary-tree/auxiliary-tree.svg)

<figcaption><em>Cây phụ được sinh ra</em></figcaption>
</figure>
</div>
...
![Hai đỉnh truy vấn không thể kết nối khi thiếu đỉnh LCA](./auxiliary-tree/disconnected-vertices.svg)

<figcaption><em>Minh hoạ việc không thể kết nối các đỉnh</em></figcaption>
</figure>
...
3. $u$ và $v$ không đứng kề nhau: giả sử $u$ đứng trước $v$ trong danh sách $A$. Kí hiệu $T_u$ là cây con gốc $u$ trong cây ban đầu, xét đỉnh $u' \in A$ là đỉnh cuối cùng trong $T_u$ và đỉnh $v' \in A$ là đỉnh đầu tiên không nằm trong $T_u$ nhưng nằm trong LCA của $u$ và $v$, đỉnh $v'$ luôn tồn tại vì $v$ là đỉnh không thuộc $T_u$. Dễ thấy thứ tự nằm trong $A$ của các đỉnh là $u \le u' \lt v' \le v$ và LCA của $u'$ và $v'$ cũng chính là LCA của $u$ và $v$. Trong trường hợp này, số lượng đỉnh LCA không thay đổi. Hình minh hoạ (số trên mỗi đỉnh là thứ tự duyệt DFS):

<figure>

![Thứ tự duyệt DFS của các đỉnh u, u', v' và v](./auxiliary-tree/dfs-order.svg)

<figcaption><em>Ví dụ minh hoạ các đỉnh u, u', v', v</em></figcaption>
</figure>
...
![Cây có số đỉnh LCA được thêm bằng k - 1](./auxiliary-tree/maximum-lca.svg)

<figcaption><em>Trường hợp số đỉnh LCA đạt tối đa</em></figcaption>
</figure>
...
![Cây phụ với thứ tự duyệt DFS ghi trên mỗi đỉnh](./auxiliary-tree/construction-overview.svg)

<figcaption><em>Cây phụ minh hoạ</em></figcaption>
</figure>
...
- Lần lượt xét qua các đỉnh còn lại (giả sử đỉnh đang xét là $u$):
  - Nếu đỉnh ở đầu stack là tiền bối của $u$ thì đỉnh này sẽ là cha trực tiếp của $u$ trong cây phụ (lí do là vì các đỉnh được duyệt theo thứ tự tăng dần trong phép duyệt DFS) do đó ta thêm cạnh nối giữa $u$ và đỉnh ở đầu stack, thêm $u$ vào stack. Ở hình minh hoạ bên dưới, do đỉnh $2$ là tiền bối của đỉnh $u = 9$ nên ta thêm cạnh nối giữa $2$ và $9$.
    ...
    ![Thêm cạnh nối đỉnh 2 ở đầu stack với đỉnh 9](./auxiliary-tree/construction-case-ancestor.svg)

    <figcaption><em>Hình minh hoạ trường hợp 1</em></figcaption>
    </figure>
  ...
  - Ngược lại, ta biết rằng một tiền bối gần với $u$ nhất sẽ nằm trong stack (đỉnh này luôn tồn tại do LCA của $u$ luôn được xét trước $u$) nên ta sẽ lấy phần tử ra khỏi stack cho đến khi phần tử ở đỉnh stack là tiền bối của $u$. Hình minh hoạ:
    ...
    ![Lấy đỉnh 9 khỏi stack để tìm tiền bối gần nhất của đỉnh 5](./auxiliary-tree/construction-case-stack.svg)

    <figcaption><em>Hình minh hoạ trường hợp 2</em></figcaption>
    </figure>
...
![Cây phụ hoàn chỉnh sau khi thêm tất cả các cạnh](./auxiliary-tree/result-tree.svg)

<figcaption><em>Cây kết quả</em></figcaption>
</figure>
...
Cho một cây có $n$ đỉnh. Một tập các đỉnh được gọi là _passable_ nếu tồn tại một đường đi đi qua tất cả các đỉnh, không có cạnh nào đi qua hai lần và đường đi này có thể đi qua các đỉnh khác (không nằm trong tập đỉnh).

Có $q$ truy vấn. Mỗi truy vấn được cho bởi một tập các đỉnh, yêu cầu hãy xác định xem tập đỉnh này có là _passable_ hay không.
...
Nhận xét rằng một tập các đỉnh là _passable_ khi mỗi đỉnh chỉ có tối đa một con hoặc tối đa hai con trong trường hợp đó là đỉnh gốc.

Với mỗi truy vấn, ta sẽ xây dựng cây phụ và kiểm tra xem cây phụ được tạo thành có là _passable_ hay không.

Chứng minh tính đúng đắn:

- Nếu một tập đỉnh là _passable_ thì việc thêm các đỉnh LCA sẽ không thay đổi đáp án do các đỉnh LCA được thêm cũng sẽ nằm trên đường đi.
- Nếu một tập đỉnh không phải là _passable_ thì việc thêm các đỉnh LCA cũng không thể khiến các đỉnh trở nên _passable_.
...
- [Codeforces — Virtual tree](https://codeforces.com/blog/entry/76955)
- [Sengxian's blog — Virtual tree](https://blog.sengxian.com/algorithms/virtual-tree)
- [OI Wiki — Virtual tree](https://oi-wiki.org/graph/virtual-tree/)
